...
This document owns the client half of VyaparPay: the Gradle module graph and the dependency rule that keeps it acyclic, the MVVM + unidirectional-data-flow pattern every screen follows, and the twelve components that turn "Rajesh taps Call Support" into a live, screen-aware voice call — the support button, the call state machine, the foreground service that holds the call, the LiveKit client wrapper, the permission flows, the on-device context capture chain, and the in-call overlay. The signature transform that produces the semantic IR (`UiTreeCollector` → `SemanticSnapshotBuilder`) is *specified* in [docs/07](07-ui-semantic-context.md); this doc covers how those components are *wired* into the app's lifecycle, threading model, and DI graph.

**Read this with:** [docs/07](07-ui-semantic-context.md) for the view-tree collection strategy and IR schema this app produces, [docs/13](13-api-contracts.md) for the REST envelope, session lifecycle, and `ui`-topic messages the client consumes, [docs/06](06-voice-pipeline.md) for the audio path and barge-in the `:voice` module plugs into, and [docs/08](08-context-and-events.md) for the event taxonomy the `EventTracker` emits.
...
| `:core:analytics` | The user-action timeline | `EventTracker` (§3.8) |
...
| `:voice` | The call itself — service, transport, state, focus | `VoiceCallService`, `WebRtcClient`, `CallStateMachine`, the `ContextChannel` adapter |

Two edges are worth defending. `:core:screencontext → :core:analytics` and `:core:screencontext → :core:network` exist because `SemanticSnapshotBuilder` reads the `EventTracker` ring buffer (for `last_action`) and the network interceptor's last non-2xx (`last_api`) — both facts no view-tree walk can produce ([docs/07 §5](07-ui-semantic-context.md) rule 7). `:voice → :core:screencontext` (not the reverse) is what keeps LiveKit out of the capture path: `ScreenContextPublisher` speaks to a `ContextChannel` *interface* declared in `:core:screencontext`, and `:voice` provides the LiveKit-backed implementation. The diff/debounce logic stays framework-free and unit-testable; only the byte-shipping adapter imports LiveKit — the same boundary discipline the backend applies to `VoiceAgentWorker` ([docs/05 §1.1](05-agent-architecture.md)).
...
| Service-held (manual) | `WebRtcClient`, `CallStateMachine`, the `ContextChannel` adapter | Call-scoped — created when `VoiceCallService` starts, torn down when the call ends; a leaked `PeerConnection` is a live mic |

`WebRtcClient` is intentionally **not** a `@Singleton`: it wraps a LiveKit `Room` whose lifetime is exactly one call. It is constructed and released by `VoiceCallService`, which owns the call's lifecycle (§3.3). Everything else that must survive across screens — the ring buffer, the aggregated app state — is a `@Singleton` interface so a screen swap never loses the timeline that the next call's context depends on.
...
**Responsibility.** The single source of truth for call state: `Idle → Requesting → Connecting → InCall → Reconnecting → Ended`. It is a pure state reducer — events in, `(state, side-effects)` out — with no LiveKit or Android imports, so it is exhaustively unit-testable.

```kotlin
sealed interface CallState { object Idle; object Requesting; object Connecting
    object InCall; object Reconnecting; data class Ended(val reason: EndReason) }

sealed interface CallEvent { object SupportTapped; data class SessionMinted(val join: JoinCredentials)
    object RoomConnected; object TransportLost; object TransportResumed
...
stateDiagram-v2
    ...
    Requesting --> Connecting: SessionMinted / WebRtcClient.connect
    Requesting --> Ended: Failed (429 / 503 / error)
    Connecting --> InCall: RoomConnected / publish mic, wire ContextChannel, request audio focus
    Connecting --> Ended: Failed (timeout / bad token)
    InCall --> Reconnecting: TransportLost / update notification + overlay indicator
    Reconnecting --> InCall: TransportResumed / resume publish, request ctx snapshot
    Reconnecting --> Ended: GraceExpired
    InCall --> Ended: UserHungUp / DELETE v1 sessions
...
| `Requesting → Connecting` | `201` from `POST /v1/sessions` | `WebRtcClient.connect(url, token)` |
| `Requesting → Ended` | `429`/`503`/network error | show "couldn't start the call"; stop FGS (no room to clean up, [docs/13 §2.1](13-api-contracts.md)) |
| `Connecting → InCall` | `Room` connected + agent joined | publish mic; subscribe agent audio; bind `ScreenContextPublisher` to the data channel; request audio focus |
| `InCall → Reconnecting` | LiveKit `reconnecting` callback | keep service + mic alive; overlay shows "reconnecting"; notification updated |
| `Reconnecting → InCall` | LiveKit `reconnected` | resume publishing; publisher sends a fresh full snapshot (seq re-sync, [docs/08 §3.3](08-context-and-events.md)) |
| `Reconnecting → Ended` | 30 s grace elapsed ([docs/06 §6](06-voice-pipeline.md)) | tear down; next attempt mints a fresh session |
| `InCall → Ended` | user taps End, or remote hangup | `WebRtcClient.disconnect`; `DELETE /v1/sessions/{id}` |
| `* → Ended` | any terminal | abandon audio focus; stop foreground service; poll `GET /v1/sessions/{id}/summary` for the summary card |

**Lifecycle.** Owned by `VoiceCallService`; dies with the call. **Threading.** All `dispatch` calls are marshalled onto a single call-scoped dispatcher so the reducer is never re-entered concurrently — LiveKit callbacks arrive on the SDK thread, UI intents on main, and both funnel through one `Channel`.

### 3.3 `VoiceCallService` (`:voice`)

**Responsibility.** The foreground service that *is* the call. It hosts `WebRtcClient`, `CallStateMachine`, and the publisher binding, so the call survives the user navigating away from `ConversationOverlay` (the minimized-chip case, §3.11). The call lives in a service, not a `ViewModel`, precisely because a `ViewModel` dies with its screen and a call must not.
...
**Notification.** An ongoing, non-dismissable call notification with mute and end-call actions (`PendingIntent`s that fire `ACTION_MUTE`/`ACTION_END` back into `onStartCommand`), plus the live call duration. On Android 13+ this notification only appears if `POST_NOTIFICATIONS` was granted; the call still runs without it (§3.5), controlled from the in-app overlay.

**Lifecycle / threading.** Started on `Idle → Requesting`, stopped on entering `Ended`. Owns a `SupervisorJob` call scope cancelled at teardown; LiveKit SDK callbacks are received on the SDK thread and forwarded to the state machine's single dispatcher.

### 3.4 `WebRtcClient` (`:voice`)

**Responsibility.** A thin wrapper over the LiveKit Android SDK `Room` (canon ADR-1). It is the only class in the app that imports LiveKit. It connects with the minted token, publishes the mic track, subscribes the agent's audio, carries the data-channel context up and the `ui` stream down, and surfaces reconnection callbacks — and it owns audio routing and focus.

```kotlin
interface WebRtcClient {
    val events: SharedFlow<RtcEvent>                 // Connected, Reconnecting, Reconnected, Disconnected, AgentAudio
    suspend fun connect(url: String, token: String)
    suspend fun publishMicrophone()
    fun setMuted(muted: Boolean)
    suspend fun sendContext(bytes: ByteArray)        // topic "ctx", reliable + ordered
    fun incoming(topic: String): Flow<ByteArray>     // topic "ui" — transcript.* / agent.state
    fun route(device: AudioRoute)                    // Earpiece | Speaker | Bluetooth
    suspend fun disconnect()
}
```

**Data channel.** `sendContext` publishes on topic `ctx` (reliable, ordered — canon §10); `incoming("ui")` exposes the server → client `transcript.partial`/`transcript.final`/`agent.state` stream ([docs/13 §4](13-api-contracts.md)). The client never gap-checks `ui`; a lost caption is repaired by the next partial.
...
**Lifecycle.** Constructed by `VoiceCallService` on start, released on `Ended` (unpublish, `Room.disconnect`, abandon focus, restore `AudioManager.mode`). **Threading.** SDK callbacks on the LiveKit thread are re-emitted onto `events` (a `SharedFlow`); consumers collect on the call scope.

### 3.5 `PermissionManager` (`:feature:support`)
...
### 3.6 `UiTreeCollector` + `SemanticSnapshotBuilder` (`:core:screencontext`)
...
### 3.7 `NavigationTracker` (`:core:screencontext`)
...
### 3.8 `EventTracker` (`:core:analytics`)
...
### 3.9 `ScreenContextPublisher` (`:core:screencontext`)

**Responsibility.** Own what leaves the device in-call: debounce, diff snapshot-vs-delta, assign client-monotonic `seq`, and hand bytes to the transport. The transport is a `ContextChannel` interface (LiveKit-backed impl in `:voice`), so the diffing logic is testable without a room.
...
### 3.10 `AppStateManager` (`:core:screencontext`)
...
### 3.11 `ConversationOverlay` (`:feature:support`)

**Responsibility.** The in-call UI: a bottom sheet with live captions, an agent-state indicator, and mute + end-call controls, rendered entirely from the `ui`-topic data-channel stream ([docs/13 §4](13-api-contracts.md)). It renders state; it never infers it.
...
**Minimized chip.** When the user navigates away mid-call (e.g. to check a settlement Asha mentioned), the overlay collapses to a small persistent chip; **the call continues in `VoiceCallService`**, capture keeps running, and tapping the chip re-expands the sheet. This is the entire reason the call lives in a service and not the overlay's `ViewModel` (§3.3). **Lifecycle / threading.** Stateless Compose bound to `CallViewModel`, which collects `WebRtcClient.incoming("ui")` and `CallStateMachine.state`; main thread.
...
| Compose UI tests | `createAndroidComposeRule`, testTags | The call flow: tap `SupportButton` → overlay appears → captions render from a fake `ui` stream → end-call tears down |
...
|---|---|---|---|---|
| Service killed by OS (memory pressure) | `START_STICKY` re-entry with null intent | The `PeerConnection` is gone with the process; the call is dead | On re-entry the service reads persisted `session_id` + start time; if within the 30 s grace ([docs/06 §6](06-voice-pipeline.md)) it drives a reconnect via a fresh room token, else transitions to `Ended` | If grace passed: overlay shows "call ended" + the summary card; the user re-taps to start fresh |
| Token expiry mid-call | `exp` reached; LiveKit needs a token only to (re)join, not to hold an established connection ([docs/13 §6](13-api-contracts.md)) | An established call is unaffected; only a *cold rejoin* after the 5 min TTL fails | On a rejoin past TTL, call `POST /v1/sessions/{id}/token` for a fresh room-scoped token — same session, same Redis state | None if the connection held; a brief re-mint round trip on a cold rejoin |
| Permission revoked mid-call | Android kills the process when `RECORD_AUDIO` is revoked in settings → looks like process death | Call is force-ended by the platform | On restart there is no live call; `PermissionManager.state` now reads denied | `SupportButton` degrades to the text-chat CTA (§3.5) until the mic permission is re-granted |
...
| `CallStateMachine` | `Idle→Requesting→Connecting→InCall→Reconnecting→Ended`, pure reducer, effects executed by the service | `:voice`, [docs/13](13-api-contracts.md) |
| LiveKit isolation | `WebRtcClient` is the only LiveKit importer; `ContextChannel` port keeps `:core:screencontext` framework-free | [docs/06](06-voice-pipeline.md), [docs/07](07-ui-semantic-context.md) |
