...
**As built (Phase-5 Batch 5) — this section is the spec, not the description.** Two provisioned boards now exist, `VyaparPay - Turn Latency` and `VyaparPay - Cost & Token Budgets`, checked in under `infra/docker/grafana/dashboards/`. **[backend/DASHBOARDS.md](../backend/DASHBOARDS.md) is the authority on what shipped**, including per-panel meaning and an explicit list of what remains unverified. Three deliberate divergences from the table above, each argued there: there is **no Loki** in the stack, so cost and per-call tokens come from the Postgres `call_costs` ledger (which is also the more correct source — the span-borne `cost_usd` is last-write-wins across a turn's LLM rounds) and the **Error rate** and **Active calls** panels have no non-Loki source and are deliberately absent rather than faked; and the stacked latency panel stacks **`llm.ttft`, not `llm.total`** as sketched above, because §4.1 of [docs/06](06-voice-pipeline.md) maps the 450/900 budget row to `llm.ttft` while `llm.total` spans the whole generation, which by design continues past the "agent audio starts" boundary the turn budget ends at — stacking it both double-counts the `tts.first_byte` overlap and measures a longer window than the budget it is read against.
