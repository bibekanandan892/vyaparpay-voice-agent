...
| TTS 5xx mid-sentence | HTTP error / stream abort on `tts.first_byte` or mid-stream from ElevenLabs | One sentence's audio cuts out or never starts | The backup-voice retry applies **only before the sentence's first audio byte** (never-starts case): `TtsProvider` retries that one sentence on the config'd fallback voice id, and because dispatch is per-sentence only the failed sentence is re-synth'd, not the whole turn. **After** audio has been yielded (cuts-out case), the provider never retries — the flat `TtsChunk` stream has no resume-from-offset, so a blind re-synthesis would double-speak the already-played prefix; mid-stream failure propagates to `VoiceAgentWorker`, which skips the sentence and continues. `AudioEgress` emits silence, not starvation, while it waits (§7) | Never-starts: a brief gap, then the sentence plays in the backup voice. Cuts-out: the rest of that sentence is skipped and the reply continues; if even the pre-first-byte retry fails, skip the sentence and continue, or offer `escalate_to_human` |
