...
| 400 | `VALIDATION_UNSUPPORTED_VERSION` | A wire *envelope* carries a version this server does not know — today that means the §6 signaling envelope's `v`. Deliberately **not** raised for a `screen_context` on `POST /v1/sessions`, which degrades rather than rejects (§2.1) | No |
...
| Endpoint | Purpose |
|---|---|
| `POST /v1/sessions` | Mint a call session: ingest the initial context best-effort (§2.1 — a bad snapshot degrades the prompt, never the call), mint the signaling token and TURN credentials, return the connect bundle |
...
The `ice_servers` array is passed verbatim to `PeerConnection` construction on the client — STUN first (credential-free), then the coturn TURN entry with its HMAC credential pair (§7). `expires` is the signaling token's connect deadline (5-min TTL, §6), not the session's lifetime. Server-side effects, in order: validate the request *envelope* (`user_id` must equal the JWT `sub` — §1.2 — and `recent_events` must be an array of `{type, name, ts}` objects within the server's defensive count cap; either failure is `400 VALIDATION_SCHEMA` before anything is minted), check `rate:usr_rajesh01` (`429 RATE_LIMITED` past 5/min), mint `a1f3c9` and create the `conversations` stub row, write `ctx:a1f3c9` and the events list, mint the one-time signaling token (stored hashed in Redis, 300 s key TTL) and compute the TURN credential pair (§7), kick off speculative context prefetch (profile + RAG on the error code), return. The two `ctx:*` writes come *after* the row exists because both are keyed by the session id the mint produces. The failed-call ergonomics matter: a `429` or `503` here means the app shows "couldn't start the call" — no session row, no token, nothing on the voice-worker to clean up, because the worker learns a session exists only when its WebSocket connects.

**A bad `screen_context` does not fail this call — it empties one prompt slot.** An unsupported `v`, a payload that fails [`protocol/schemas/screen_context.v1.json`](../protocol/schemas/screen_context.v1.json), one that is oversized past what the [docs/07 §7](07-ui-semantic-context.md) drop ladder can recover, or a Redis blip while storing it: every one of these returns `201` with the connect bundle, and the only consequence is that slot 4 of the prompt reads back empty — byte-identical to a client that honestly sent `screen_context: null`. The same holds for the `recent_events` write and slot 5. This is deliberate and it is the *opposite* of what an earlier draft of this section specified (`400 VALIDATION_SCHEMA` / `VALIDATION_UNSUPPORTED_VERSION`, "whole body, never partial ingest"); the code is right and the promise was wrong, so the promise moved.

The reasoning, from [docs/08 §7](08-context-and-events.md)'s closing rule — *the pipeline degrades context, never conversation*: rejecting the body punishes the merchant for the app build on their phone. A stale client that still speaks `screen_context/v0`, a capture corrupted by a Compose edge case, a transient Redis timeout — under a rejecting contract each of those is a merchant who **cannot start a support call at all**, at the exact moment they have already hit a payment failure and pressed Call Support. Under the degrading contract they get the call, and Asha simply opens without the screen in front of her and asks instead of knowing. Losing context costs one clarifying question; losing the call costs the whole interaction. Note also that by the time the snapshot is examined the `conversations` row is already committed, so a late rejection could not be a clean `400` anyway — it would be a `500` on a session that already exists.

"Never partially ingest" *does* survive, and is the one part of the old text that was never about status codes: a snapshot that fails validation is discarded **whole**, never merged field-by-field into `ctx:{id}`. Garbage must not enter the prompt; that rule is enforced by dropping the payload, not by failing the request. Implementation: `_ingest_initial_screen_context` in `backend/app/api/routes/sessions.py`, whose docstring carries the same reasoning at the call site.
