...
class ToolExecutor:
    ...
    async def _audit(
        ...
        *,
        ...
        Two independent security reviews on this parameter converged on
        the same MEDIUM: `except IntegrityError` alone doesn't distinguish
        `idempotency_key`'s UNIQUE violation from `tool_invocations`'
        other constraints (its `session_id` FK, two CHECKs —
        `app/models/orm.py`'s `ToolInvocation.__table_args__`) — asyncpg
        maps every constraint-violation SQLSTATE class to the same
        `IntegrityError` wrapper, so catching the wrapper type alone
        can't tell them apart, and a future migration adding a new
        constraint would silently start losing diagnostic detail (this
        branch logs at info, with no `exc_info`) for a genuinely
        different failure. Fixed by checking the DRIVER-level exception
        `IntegrityError.orig` wraps: `asyncpg.exceptions.
        UniqueViolationError` is asyncpg's own type for SQLSTATE 23505
        specifically, independent of the constraint's NAME (verified
        empirically NOT to be the ORM's `_NAMING_CONVENTION`-derived
        `uq_tool_invocations_idempotency_key` — `migrations/versions/
        0001_initial_schema.py`'s `idempotency_key` column is a
        hand-written raw `sa.Column(..., unique=True)` inside
        `op.create_table`, which gets Postgres's own default
        `tool_invocations_idempotency_key_key`, not the ORM naming
        convention, since 0001 predates and isn't autogenerated from the
        ORM model — checking the exception TYPE instead of a specific
        name sidesteps that mismatch entirely). `idempotency_key` is the
        only UNIQUE column on this table, so a `UniqueViolationError` at
        this call site is unambiguous."""
        try:
            async with self._sessionmaker() as session:
                row = await ToolAuditRepo(session).insert(
                    session_id=session_id,
            ...
            if (
                ...
                and isinstance(exc.orig, asyncpg.exceptions.UniqueViolationError)
            ):
                log.info(
